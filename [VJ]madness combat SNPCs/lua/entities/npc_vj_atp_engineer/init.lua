...
-----------------------------------------------*/
ENT.Model = {"models/madness/npc/atp_engineer_npc.mdl"} -- The game will pick a random model from the table when the SNPC is spawned | Add as many as you want
ENT.StartHealth = 60
ENT.melee = 0
...
function ENT:CustomOnTakeDamage_OnBleed(dmginfo, hitgroup) 
	...
	if dmgType == DMG_CLUB && dmginfo:IsDamageType(DMG_CLUB) and self:Health() < (self:GetMaxHealth() * 0.7) then 
		if self:GetBodygroup(5) == 2 then
            return
        end	
	...
	if dmgType == DMG_CLUB && dmginfo:IsDamageType(DMG_CLUB) and self:Health() < (self:GetMaxHealth() * 0.4) then 
		if self:GetBodygroup(5) == 2 then
            return
        end	
	...
	self:CreateGibEntity("prop_physics","models/madness/npc/gibs/masks_part1.mdl",{Pos=self:LocalToWorld(Vector(30,0,54)),Ang=self:GetAngles()+Angle(0,0,0),Vel=self:GetRight()*math.Rand(350,350)+self:GetForward()*math.Rand(-200,300)})
	self:CreateGibEntity("prop_physics","models/madness/npc/gibs/masks_part2.mdl",{Pos=self:LocalToWorld(Vector(-30,0,54)),Ang=self:GetAngles()+Angle(0,0,0),Vel=self:GetRight()*math.Rand(0,-350)+self:GetForward()*math.Rand(600,300)})
	self:CreateGibEntity("prop_physics","models/madness/npc/gibs/masks_part3.mdl",{Pos=self:LocalToWorld(Vector(30,0,60)),Ang=self:GetAngles()+Angle(0,0,0),Vel=self:GetRight()*math.Rand(350,350)+self:GetForward()*math.Rand(-200,300)})
	self:CreateGibEntity("prop_physics","models/madness/npc/gibs/masks_part4.mdl",{Pos=self:LocalToWorld(Vector(-30,0,60)),Ang=self:GetAngles()+Angle(0,0,0),Vel=self:GetRight()*math.Rand(0,-350)+self:GetForward()*math.Rand(600,300)})
	...
	if hitgroup == HITGROUP_GEAR and self.totalDamage[hitgroup] > 13000	 then    -- Dismember heads code
		if self:GetBodygroup(5) == 2 then
            return
        end	
		local head_damege_test = math.random(1,2)
